...
export default function TagPortfolioList({ tagName, pageTitle, pageDescription }: TagPortfolioListProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const itemsWithStats = validItems.map((item: { id: string; title: string; image_url: string; thumbnail_url: string | null; creator_id: string; category: string; tags: string[]; created_at: string }) => ({
        ...item,
