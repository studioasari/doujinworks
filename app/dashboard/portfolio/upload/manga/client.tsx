...
function UploadMangaContent() {
  ...
  const presetTags = [
    ...
    '少年漫画', '少女漫画', '青年漫画', 'BL', 'GL',
    'ギャグ', 'シリアス', '日常', 'バトル', 'ラブコメ',
    'ホラー', 'ミステリー', '4コマ', '1話完結', '連載'
  ]
    ...
    try {
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        try {
          ...
          console.error(`${i + 1}ページ目エラー:`, uploadError)
          throw new Error(`${i + 1}ページ目のアップロードに失敗しました`)
        }
      }
...
export default function UploadMangaClient() {
  return <UploadMangaContent />
}