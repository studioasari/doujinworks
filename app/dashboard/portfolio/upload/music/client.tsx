...
function UploadMusicContent() {
  ...
  const presetTags = [
    'オリジナル曲', 'カバー', 'アレンジ', 'インスト', 'ボーカル',
    'ポップ', 'ロック', 'エレクトロ', 'ジャズ', 'クラシック',
    'アニソン', 'ボカロ', 'アコースティック', 'バラード', 'アップテンポ',
    '癒し', '切ない', '明るい', 'かっこいい', 'かわいい'
  ]
  ...
  function restoreDraft(draftId: string) {
    try {
      const saved = localStorage.getItem('music_drafts')
      if (saved) {
  ...
  function deleteDraft(draft: Draft) {
    try {
      const storageKey = draft.category || 'music_drafts'
  ...
  useEffect(() => {
    ...
    const autoSaveTimer = setTimeout(() => {
      try {
        ...
        localStorage.setItem('music_drafts', JSON.stringify(allDrafts))
        recount()
      } catch (error) {
  ...
  function handlePreSubmit(e: React.FormEvent) {
    ...
    if (uploadMethod === 'file' && !audioFile) {
      setErrors(prev => ({ ...prev, audio: '音楽ファイルを選択してください' }))
      setToast({ message: '音楽ファイルを選択してください', type: 'error' })
      return
    }
    ...
    try {
      ...
      if (uploadMethod === 'file' && audioFile) {
        try {
          ...
          console.error('音楽ファイルエラー:', uploadError)
      ...
      setToast({ message: '音楽をアップロードしました！', type: 'success' })
    ...
    <>
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <h1 className={styles.title}>音楽をアップロード</h1>
          <button
            type="button"
        ...
        <form onSubmit={handlePreSubmit} className={styles.form}>
          ...
          <div className={styles.section}>
            <label className="form-label">
              音源のアップロード方法 <span className={styles.required}>*</span>
            </label>
        ...
        <div className="modal-overlay active" onClick={() => setShowConfirmModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '600px' }}>
            ...
            <div className="modal-body">
              ...
              <div className={styles.confirmItem}>
                <span className={styles.confirmLabel}>音源</span>
                {uploadMethod === 'file' ? (
              ...
              <div className={styles.confirmItem}>
                ...
                <span className={styles.confirmValue}>{isOriginal ? 'オリジナル作品' : 'カバー・アレンジ'}</span>
              </div>
