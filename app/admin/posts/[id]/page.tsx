...
export default function AdminEditPostPage() {
  ...
  const handleSubmit = async (publish?: boolean) => {
    ...
    if (publish === true) {
      ...
      if (!formData.published_at) updateData.published_at = new Date().toISOString()
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {formData.status === 'published' ? (
            <>
              <button className="admin-action-btn warning" onClick={() => handleSubmit(false)} disabled={saving} style={{ width: '100px', padding: '8px 0', fontSize: '0.8125rem' }}>
                非公開
              </button>
              <button className="admin-action-btn primary" onClick={() => handleSubmit()} disabled={saving} style={{ width: '100px', padding: '8px 0', fontSize: '0.8125rem' }}>
                {saving ? '保存中...' : '更新する'}
              </button>
            </>
          ) : (
            <>
              <button className="admin-action-btn secondary" onClick={() => handleSubmit()} disabled={saving} style={{ width: '100px', padding: '8px 0', fontSize: '0.8125rem' }}>
                下書き保存
              </button>
              <button className="admin-action-btn primary" onClick={() => handleSubmit(true)} disabled={saving} style={{ width: '100px', padding: '8px 0', fontSize: '0.8125rem' }}>
                公開する
              </button>
            </>
          )}
        ...
        <div style={{ width: '280px', background: '#fafafa', overflowY: 'auto', flexShrink: 0 }}>
          ...
          <div style={{ padding: '16px', borderBottom: '1px solid #e5e7eb' }}>
            ...
            <div style={{
              ...
              fontSize: '0.8125rem',
              fontWeight: '500',
              background: formData.status === 'published' ? '#dcfce7' : '#f3f4f6',
              color: formData.status === 'published' ? '#16a34a' : '#6b7280'
            }}>
              <i className={`fas ${formData.status === 'published' ? 'fa-globe' : 'fa-file-alt'}`} style={{ fontSize: '0.75rem' }}></i>
