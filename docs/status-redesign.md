...
3. **ラベルのゆれ**: 同一ステータス値(`contracted`)が画面によって「決済待ち」「募集終了」「契約済み」の3通りに表示されている。
...
[作成]
  │
  ↓
[contracted] ── 決済完了 ──→ [paid] ── 納品 ──→ [delivered] ── 検収承認 ──→ [completed]
  │                                   │                   │
  │                                   │                   └── 差戻し ──→ [paid]
  │                                   │
  └─── キャンセル ──→ [cancelled] ──┘
...
4. [ ] クライアントで決済 → work_contract.status=paid
